...
file = "/media/hodaya/DISK_IMG/RPDR/RPDR/Pathology_(2).xml"
...
def deidentification(data):
    ...
    for line in data:
        for i in line:
            ...
            i = re.sub(r'[a-z]{2,10}\s[a-z]{1,7}\s[a-z]{3,11}\s(md\b|m\sd\b|ct\sascp\b|ct\b|cnp\b|md\smba\b)', r'PHYSICIAN_NAME',i) # firstname m lastname 'md'/'m d'/'ct ascp'/'ct'
            i = re.sub(r'[a-z]{3,10}\s[a-z]{4,10}\s[a-z]\s(md\b|m\sd\b|ct\sascp\b|ct\b|cnp\b|md\smba\b)', r'PHYSICIAN_NAME',i) # lastname firstname m 'md'/'m d'/'ct ascp'/'ct'
            i = re.sub(r'[a-z]{3,10}\s[a-z]{3,10}\s(md\b|m\sd\b|ct\sascp\b|ct\b|md\smba\b)', r'PHYSICIAN_NAME',i) # firstname lastname 'md'/'m d'/'ct ascp'
            ...
            i = re.sub(r'(?<=PATIENT_NAME\s)(?=\sdob)', r'TEST', i)
            #i = re.sub(r'(?<=PHYSICIAN_NAME\s)|(?<=results\sto\s)(?=\sPHYSICIAN_NAME)', r'TEST', i)
            ...
            i = re.sub(r'(dr\s|mr\s|ms\s|mrs\s)[a-z]{4,10}\s[a-z]\s[a-z]{4,8}', r'NAME', i) # dr/mr/ms/mrs firstname m lastname
    ...
    for r in range(49):
        ...
        if re.search(r'[a-z]{2,10}\s[a-z]{1,7}\s[a-z]{3,11}\s(md\b|m\sd\b|ct\sascp\b|ct\b|cnp\b|md\smba\b)', report) != None: # firstname m lastname 'md'/'m d'/'ct ascp'/'ct'
            return "A case that wasn't deidentified was caught (1)"
        elif re.search(r'[a-z]{3,10}\s[a-z]{4,10}\s[a-z]\s(md\b|m\sd\b|ct\sascp\b|ct\b|cnp\b|md\smba\b)', report) != None: # lastname firstname m 'md'/'m d'/'ct ascp'/'ct'
            return "A case that wasn't deidentified was caught (2)"
        elif re.search(r'[a-z]{3,10}\s[a-z]{3,10}\s(md\b|m\sd\b|ct\sascp\b|ct\b|md\smba\b)', report) != None: # firstname lastname 'md'/'m d'/'ct ascp'/'ct'
